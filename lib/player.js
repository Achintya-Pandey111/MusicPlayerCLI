...
function play(songObj, onEndedCallback) {
  // If a song is already playing or paused, stop it first
  ...
  try {
    ...
    activeChildProcess.on('exit', (code, signal) => {
      ...
      // Defensive check for natural song completion:
      // 1. Was NOT manually stopped by the user.
      // 2. Process exited successfully (code 0 or null).
      // 3. Played for at least 1.5 seconds (prevents rapid infinite loops on invalid/dummy audio files).
      ...
      if (!wasManual && exitedSuccessfully && playedLongEnough) {
        ...
        console.log(`\n[Playback Notice] Track exited prematurely after ${durationMs}ms. Skipping auto-next.`);
      }
    });
